...
# This file is part of nautilus-dropbox 2018.11.08.
#
...
def download_file_chunk(url, buf):
    ...
    opener.addheaders = [('User-Agent', "DropboxLinuxDownloader/2018.11.08")]
...
class DownloadState(object):
    ...
    def is_dropbox_valid(self):
        ...
        f = open("/dev/null", "w")
        try:
            a = subprocess.Popen([db_path, "/testrun", "0"], preexec_fn=os.setsid, cwd=os.path.expanduser("~"), stderr=sys.stderr, stdout=f, close_fds=True)
        except Exception, ex:
            print ex
            return False
...
class DropboxCommand(object):
    ...
    def send_command(self, name, args):
        ...
        self.f.write(u"\n".encode('utf8'))
        self.f.writelines((u"\t".join([k] + (list(v) if hasattr(v, '__iter__') else [v])) + u"\n").encode('utf8') for k, v in args.iteritems())
        self.f.write(u"done\n".encode('utf8'))
...
def start_dropbox():
    ...
    if os.access(db_path, os.X_OK):
        ...
        a = subprocess.Popen(["dbus-launch", db_path], preexec_fn=os.setsid, cwd=os.path.expanduser("~"), stderr=sys.stderr, stdout=f, close_fds=True)
            ...
            if options.list:
                ...
                for file in args:
                    ...
                    if not os.path.exists(fp):
                        console_print(u"%-*s %s" % (indent, file + ':', "File doesn't exist"))
                        continue
...
def proxy(args):
    ...
    if len(args) == 0 or mode not in [u'none', u'auto', u'manual'] or (mode == 'manual' and len(args) not in (4, 6)) or (mode != 'manual' and len(args) != 1) or (mode == 'manual' and type_ not in [u'http', u'socks4', u'socks5']):
        # Print help
